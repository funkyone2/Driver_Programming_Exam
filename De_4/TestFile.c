...
void readDataDec(int fd){
	char buf[1024];
	int ret = ioctl(fd,CONVERT_BIN_TO_DEC,buf);
	if(ret < 0){
...
void readDataOct(int fd){
	char buf[1024];
	int ret = ioctl(fd,CONVERT_BIN_TO_OCT,buf);
	if(ret < 0){
		printf("Read data error !!!!\n");
	}
	else{
		printf("Du lieu o dang Oct : %s\n",buf);
	}
}
...
void menu(int fd){
	...
	while(1){
		...
		printf("1. Nhap vao so he 2\n");
		printf("2. Chuyen so he 2 sang he 10\n");
		printf("3. Chuyen so he 2 sang he 8\n");
