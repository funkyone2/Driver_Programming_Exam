...
static void bubble_sort(char *ret){
	if(flagWriteText == 0){
		...
		for(i=0;i<len_i;i++){
			for(j=i+1;j<len;j++){
				if(text_bubble[i]>text_bubble[j]){
					tmp = text_bubble[i];
					text_bubble[i]=text_bubble[j];
					text_bubble[j]=tmp;				
				}
			}
		}
...
static void showTime(char *ret){
	if(flagBubbleSort == 0){
		ret = "Chua thuc hien bubble sort !!!!\n";
	}
...
static long device_ioctl(struct file *filp,unsigned int cmd,unsigned long arg){
	...
	switch(cmd){
		...
		case BUBBLE_SORT:
			...
			bubble_sort(kernel_buf);
			ktime_get_coarse_real_ts64(&wt_end_bubble);
			printk("BUBBLE_SORT : %s",kernel_buf);
			printk("");
